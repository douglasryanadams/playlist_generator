<template>
  ...
  <div class="p-inputgroup">
    <InputText v-model="songSearch" placeholder="Artist or Song" @keyup.enter="search"/>
    <Button label="Search" @click="search"/>
  </div>

  <DataTable v-if="songStore.searchResults.length > 0" :value="songStore.searchResults" tyleStyle="min-width: 50rem">
    <Column header="Add">
      <template #body="slotProps">
        <Button label="Add" @click="songStore.selectTrack(slotProps.data)" severity="success" raised/>
      </template>
    </Column>

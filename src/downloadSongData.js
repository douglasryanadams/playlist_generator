...
export const downloadSongData = async (startTime, endTime, spotifyToken) => {
    ...
    while (next_url != null && !foundLastTrack) {
        ...
        if (response.status > 299) {
            throw {response: response, message: "Error downloading recently played. Please sign out and sign back in to try again."}
        }
        ...
        if (trackFeaturesResponse.status > 299){
            throw {response: trackFeaturesResponse, message: "Error downloading recently played song features. Please sign out and sign back in to try again."}
        }
